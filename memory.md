...
- **🔴 2026-08-25: the contact form had been accepting enquiries and silently losing them since ~08-17 — fixed, live, verified.** Bot volume on the `Work With Us` subject degraded `empowrcic.org`'s sending reputation; the **team notification** stopped being delivered while the **visitor confirmation** still went out. It was invisible because the Resend SDK returns `{ error }` rather than throwing and neither `emails.send()` call checked it, so the function returned `success: true` for enquiries nobody would see. Three fixes in `contact.ts`: both sends now check `.error` (failed team notification → 500, not a false success), Cloudflare Turnstile verified server-side, and email-format validation before any send. Commits `b7846ba`, `457462c`. **Two things to carry forward:** (1) **Turnstile must stay explicitly rendered** via `@marsidev/react-turnstile` — the implicit `cf-turnstile` class scans the DOM once on script load, so client-side nav away from `/contact` and back leaves no widget, no token, and the backend now *rejects* tokenless posts; this was caught in a real browser, not by typecheck or build. (2) `TURNSTILE_SECRET_KEY` is set in Netlify's **production context only** — deliberate, so previews skip verification rather than failing closed. Full detail: DEVLOG 2026-08-25.
...
- Contact form: currently direct-Resend-only (CRM routing temporarily reverted 2026-07-27, see Current Phase) — `contact.ts` supports both paths, toggled by whether `CRM_CONTACT_API_URL`/`CRM_CONTACT_API_KEY` are set on Netlify; visitor auto-reply always via Resend either way. **Those two vars are not currently set on Netlify at all** (verified 2026-08-25 with `netlify env:get` — they exist only in local `src/.env.local`, and the var list further down this file previously implied otherwise). Also serves cross-origin submissions from EELA's Private Bookings enquiry modal since 2026-08-20 (CORS allow-list) — same function, same behavior, not a separate integration to reason about. **Since 2026-08-25 it also requires a Cloudflare Turnstile token in production** — any new client posting to this function must send one, or it gets a 400
